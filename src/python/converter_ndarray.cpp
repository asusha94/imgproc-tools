...
#else
    UMatData* allocate(
        int dims0, const int* sizes, int type, void* data, size_t* step, int flags, UMatUsageFlags usageFlags) const
#endif
    {
        if (data != 0)
        {
...
bool NDArrayConverter::toMat(PyObject* o, Mat& m)
{
    ...
    if (needcopy)
    {
        // if (info.outputarg)
        //{
        //    failmsg("Layout of the output array %s is incompatible with cv::Mat (step[ndims-1] != elemsize or step[1]
        //    != elemsize*nchannels)", info.name); return false;
        //}
